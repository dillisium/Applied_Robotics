...
function distanceToWarning(distance) {
    if (distance > 30)
        return 0;
    else if (distance < 10)
        return 1;
...
const receivers = {
    "firmware/version": (data, message, dataView) => {
        ...
        if (dataView.byteLength > 28) {
            ...
            data.robot.obstacles = data.robot.obstacles.map((v, i) => dataView.getUint8(i + 30));
            if (data.robot.mode === 2) {
                if (data.robot.gaitType === 2)
                    data.robot.state = "run";
                else if (data.robot.gaitType === 3)
                    data.robot.state = "climb";
                else if (data.robot.gaitType === 1)
        ...
        if (dataView.byteLength >= 44) {
            ...
            if (name != "")
                data.robot.sn.product = name + "_" + model;
